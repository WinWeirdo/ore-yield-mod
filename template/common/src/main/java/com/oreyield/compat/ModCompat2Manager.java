...
public final class ModCompat2Manager {
    ...
    /** Rebuilds the curated lists after registries and tags are available. */
    ...
    public static String inferDimensionForOre(ResourceLocation oreId) {
        // Curated entries declare their actual target dimensions in ORE_SPECS.
        return "";
    }
    ...
    private static void scanOres() {
        BuiltInRegistries.BLOCK.forEach(block -> {
            ...
            ResourceLocation dropId = ResourceLocation.tryParse(spec.dropItem);
            if (dropId == null || !BuiltInRegistries.ITEM.containsKey(dropId)) {
                org.slf4j.LoggerFactory.getLogger("ore_yield/ModCompat2")
                        .warn("[Ore Yield] Skipping curated ore {} because its configured result item {} is unavailable.",
                                blockId, spec.dropItem);
                return;
            }
