...
It registers the anti-cheese provenance blocks and Stone Generator content, which Minecraft must synchronize to clients. Restart the server and all modded clients after changing it. Leave it `false` to keep Ore Yield client-optional.

### Key Settings

### Stone Generator

The Stone Generator is available only when `enable_anti_cheese_mechanics = true`; that mode requires Ore Yield on every client. It is crafted with eight diamonds around one end stone. After each cooldown, it attempts to create a host block immediately above itself: stone in the Overworld, netherrack in the Nether, and end stone in the End. It never replaces an occupied block. Its output is persistently marked, so it can be balanced independently from naturally generated or player-placed host blocks.
...
# Default: false. Enables anti-cheese markers and Stone Generator content; requires Ore Yield on all clients.
