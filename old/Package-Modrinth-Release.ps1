...
stable, upload-ready names such as ore-yield-1.20.1-forge.jar. The script
uses mod_version from gradle.properties only to locate the current build
outputs; the packaged filenames deliberately omit that version.

.EXAMPLE
.\old\Package-Modrinth-Release.ps1

.EXAMPLE
.\old\Package-Modrinth-Release.ps1 -WhatIf
#>
...
$plans = foreach ($target in $targets) {
    ...
    [pscustomobject]@{
        ...
        Source = $source
        Destination = Join-Path $DestinationDirectory "ore-yield-$($target.Minecraft)-$($target.Loader).jar"
    }
}
